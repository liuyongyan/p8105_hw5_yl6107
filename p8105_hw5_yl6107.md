...
2.  **Rejected samples only (coral line)**:
    ...
    - For small effect sizes (mu = 1, 2), the rejected-sample average
      ...
      sample means.
    - As $\mu$ increases (mu ≥ 4), the rejected-sample averages converge
      toward the true values because power is so high that nearly all
      samples lead to rejection, eliminating selection bias.
